...
run_pg <- "Prince George" %in% bulletin_regions$region
```
...
m <- leaflet::leaflet() |>
  ...
  if (length(params$warningLevel$yellow) > 0) {
    m <- leaflet::addPolygons(m,
      data = eccc_map_env[which(eccc_map_env$NAME %in% params$warningLevel$yellow), ],
      fillOpacity = 0.65,
      opacity = 0.65,
      color = "black",
      weight = 1.75,
  ...
  if (length(params$warningLevel$orange) > 0) {
      m <- leaflet::addPolygons(m,
        data = eccc_map_env[which(eccc_map_env$NAME %in% params$warningLevel$orange), ],
        fillOpacity = 0.65,
        opacity = 0.65,
        color = "black",
        weight = 1.75,
  ...
  if (length(params$warningLevel$red) > 0) {
    m <- leaflet::addPolygons(m,
      data = eccc_map_env[which(eccc_map_env$NAME %in% params$warningLevel$red), ],
      fillOpacity = 0.65,
      opacity = 0.65,
      color = "black",
      weight = 1.75,
