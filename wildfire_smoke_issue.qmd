...
a <- match_eccc_health |>
  filter(eccc %in% params$allRegions) |>
...
m <- leaflet::leaflet() |>
  ...
  if (length(params$yellowRegions) > 0) {
    m <- leaflet::addPolygons(m,
      data = eccc_map_env[which(eccc_map_env$NAME %in% params$yellowRegions), ],
      fillOpacity = 0.65,
      opacity = 0.65,
      color = "black",
      weight = 1.75,
  ...
  if (length(params$orangeRegions) > 0) {
      m <- leaflet::addPolygons(m,
        data = eccc_map_env[which(eccc_map_env$NAME %in% params$orangeRegions), ],
        fillOpacity = 0.65,
        opacity = 0.65,
        color = "black",
        weight = 1.75,
  ...
  if (length(params$redRegions) > 0) {
    m <- leaflet::addPolygons(m,
      data = eccc_map_env[which(eccc_map_env$NAME %in% params$redRegions), ],
      fillOpacity = 0.65,
      opacity = 0.65,
      color = "black",
      weight = 1.75,
