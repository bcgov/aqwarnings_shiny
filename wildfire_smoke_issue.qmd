...
# set logo path based on output format... this doesn't work (yet); but keeping here for future work
# output_format <- knitr::pandoc_to()
# if(output_format == "PDF"){
  # logo_path <- " logo](https://github.com/bcgov/aqwarnings/tree/main/frontend/assets/logo_" # DOES NOT WORK, PDF WON'T COMPILE
  logo_path<-" logo](C:/HagaR/aqwarnings/frontend/assets/logo_" #PDF WILL COMPILE WHEN LOCAL PATH IS USED
# } else {
   # logo_path <- " logo](//assets/logo_" # DOES NOT WORK PDF WON'T COMPILE
# }
...
for(logo_name in names(logos_selected)) {
   logo_image_line <- c(logo_image_line, paste0("![", logo_name, logo_path, logos_selected[[logo_name]], ".png)\\"))
}
```
