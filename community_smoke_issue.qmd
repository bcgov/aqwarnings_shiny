---
title: "`r paste0(
  'Air quality warning ', ifelse(params$burnRestrictions != 'No', ' and open burning restrictions ', ''),
  ifelse(params$ice == 'Continue', 'continued',
    ifelse(params$ice == 'End', 'ended',
      ifelse(params$ice == 'Issue', 'issued', params$ice)
    )
  ),
  ' for ', params$station)`"
...
if (params$pollutant == "PM25") {
  ...
  first_paragraph <- paste0("The Ministry of Environment and Parks in collaboration with the ", HAauth, " issued an Air Quality Warning for ", params$station, " on ", format(as.Date(params$issuedate), "%B %d, %Y"), ". The warning remains in effect due to elevated ", pollutantExpansion, "."
                            )
}
...
} else if (params$pollutant == "PM25 & PM10") {
  pollutantExpansion <- "fine particulate matter and coarse particulate matter"
...
for (i in seq_along(pollutants)) {
  ...
  if (length(valid_stations) == 0) {
    stop(sprintf("None of the specified stations are valid for %s.", pollutant))
  }
...
if (params$burnRestrictions != 'No') {
  cat(
    "Open burning restrictions are now in effect for ", params$customMessageBanArea,
...
[Latest air quality data] {{< card_end >}} {{< card_start title="Metro Vancouver Air Quality" variant="success" logo="/assets/icon_chart.svg" >}} Advisories and map of air quality and weather data.
