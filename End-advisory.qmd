...
aq_mets <- fread(file.path("contacts", "aq_mets_contact.csv"))

healthAuth <- fread(file.path("contacts", "health_auth_contact.csv"))

buddy_stations <- fread(file.path("utility", "buddy_stations.csv"))

Health_Authority <- fread(file.path("utility", "Health_Authority.csv"))
```
...
station_HAcontact <- Health_Authority %>%
  filter(Station == params$station) %>%
  pull(Health_authority)

HAauth <- healthAuth |>
       filter(nickname == station_HAcontact) |>
       mutate(station_HAcontact = paste(authority)) |>
       pull(station_HAcontact)

HAcontact <- healthAuth |>
       filter(nickname == station_HAcontact) |>
       mutate(station_HAcontact = paste(authority, primary_contact, secondary_contact, tertiary_contact, sep = "<br />")) |>
       pull(station_HAcontact)
```
