...
bylaws <- build_bylaw_information(params$location)
```
...
Exposure to `r pollutantExpansion` is particularly a concern for infants, older adults, individuals with chronic
...
if (params$pollutant == "PM10" && current_month %in% 6:8) {
...
`r if (!is.null(bylaws) && nzchar(bylaws)) paste("-", bylaws)`
