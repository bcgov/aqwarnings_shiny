...
if (params$burnRestrictions > 0) {
 ...
 ###TEMPORARY - until SDM signature is sorted out; see commented code above for long-term solution. 
    burn_ban_md_temp <- paste0(
    "As pollution is occurring or is likely to occur from open burning, pursuant to Sections 30(1) and 30(2) of the Open Burning Smoke Control Regulation, the Director has prohibited open burning ",
    params$customMessageBanArea,
    " No vegetative debris may be ignited or added to ignited piles. Contravention of these provisions may be subject to a fine under the Regulation.\n\n")
} else {
...
```{r}
#| label: create-pdf-filepath

### TEMPORARY - until SDM signature is sorted
...
# create file path for PDF output
 ...
 # Clean location name for file name
      ...
      if (params$burnRestrictions < 1) {  # no burn restriction
        ...
        output_file_name <- sprintf("%s_%s_%s", params$ice, pollutant_clean, location_clean) 
        
      } else { # burn restriction; obp = open burning prohibition
        
        output_file_name <- sprintf("%s_%s_%s_%s", params$ice, pollutant_clean, "obp", location_clean) 
        
      }
       
      pdf_file_name <- paste0(as.character(today), "_", output_file_name, ".pdf")
      
      pdf_path <- paste0("//warnings/", as.character(today), "_", output_file_name, ".pdf")
      mandatory_text <- paste0("Please see the ", "[PDF version of this Air Quality Warning](",pdf_path, ") for the statutory decision-maker sign-off.")
