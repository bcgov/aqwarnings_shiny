...
if (params$burnRestrictions %in% c("Yes - Arvind", "Yes - Ben")) {
  ...
  burn_ban_first_paragraph <- paste0(
    "Open burning restrictions are now in effect ", 
...
`r if (!is.null(burn_ban_first_paragraph) && nzchar(burn_ban_first_paragraph)) paste0(burn_ban_first_paragraph)`
...
The next update will be on The next update will be on `r format(Sys.Date() + 1, "%B %d, %Y")` and posted to the province’s [Air Quality Warnings webpage](https://www.gov.bc.ca/airquality).
