...
params: 
  ...
  burnRestrictionStatus: 0 # 0 = warning ended (no burn restrictions); 1 = warning and burn restrictions ended; 2 = warning ended; burn restrictions continue
...
# create file path for PDF output
 ...
 # Clean location name for file name
      ...
      if (params$burnRestrictionStatus == 0) {  # no burn restriction
        
        output_file_name <- sprintf("%s_%s_%s", "end", pollutant_clean, location_clean) 
        mandatory_text <- NULL
        
      } else if(params$burnRestrictionStatus == 1) { # warning and open burn restrictions end
        
        output_file_name <- sprintf("%s_%s_%s_%s", "end", pollutant_clean, "obr", location_clean) 
        mandatory_text <- NULL
        
      } else { # warning ends open burn restrictions continue
        ...
        mandatory_text <- paste0("Please see the ", "[PDF version of this Air Quality Warning](",
                                 pdf_path, 
...
`r if (params$burnRestrictionStatus < 2 | params$outputFormat == "pdf") ":::{.content-hidden}"`

{{< inline_alert_start>}} `r mandatory_text` {{< inline_alert_end >}}

`r if (params$burnRestrictionStatus < 2 | params$outputFormat == "pdf") ":::"`
...
if (params$burnRestrictionStatus == 2) {
  ...
  cat("# Mandatory emission reduction actions\n")
}
