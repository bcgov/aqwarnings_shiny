---
title: "`r paste0(
  'Air quality warning ended for ', params$location)`"  # removed "open burning restrictions" - requires user input, which doesn't exist; discuss with team if inclusion of open burning restrictions is required for "end" warnings
...
customMessageBanArea: "`r params$customMessageBanArea`"  # confirm if this field is needed for "end" warnings
...
additional_info_PM10 <- paste0(
  "The provincial air quality objective for PM$_{10}$ is 50 micrograms per cubic metre ($\\mu$g/m$^{3}$), averaged over 24 hours. 24-hour average PM$_{10}$ concentrations are summarized below for ", params$location, " and nearest monitored communities at ", format(floor_date(Sys.time(), "hour"), "%I:%M %p"), " local time today:"
  )
